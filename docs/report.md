...
Ryunosuke Tago, SUNY Geneseo, July 2026
...
This report implements and validates Fuchs-Sondheimer surface-scattering and
Mayadas-Shatzkes grain-boundary-scattering models for size-dependent Cu
interconnect resistivity. The main result is that grain-boundary scattering
dominates the measured linewidth/thickness dependence, while the fitted
grain-boundary reflection coefficient is system-specific: the Steinhogl wire
fit does not transfer directly to the Yarimbiyik film dataset. The report is
intended as a complete undergraduate research write-up and as a basis for
advisor feedback on whether the work could be developed toward publication.
...
quantifies parameter sensitivity, evaluates the second dataset using its own
literature parameters, and then performs a stricter cross-dataset transfer
test by applying the grain-boundary reflection coefficient fitted from the
first dataset without refitting.
...
![Figure 1. Reproduction of the published Steinhogl Fig. 3 curves.](../figures/benchmark_fig3.png)
...
![Figure 2. Fit to the digitized Steinhogl Cu wire dataset.](../figures/fit_steinhogl.png)
...
R = 0.50) lies within our 1σ region, so our fit is statistically consistent
with theirs. Within the Steinhögl dataset, R is more robustly constrained
than p. However, the cross-dataset transfer test in § 7 shows that R should
not be interpreted as a universal or directly transferable material
constant.
...
![Figure 3. No-refit validation on the Yarimbiyik Cu film dataset.](../figures/validate_yarimbiyik.png)

![Figure 4. Cross-dataset transfer test using the Steinhogl-fitted R value.](../figures/validate_yarimbiyik_transfer.png)
...
(§ 6). Using Yarimbiyik's literature value R = 0.32 without refitting gives an
RMSE of 0.537 µΩ·cm and a mean relative error of 7.4 %. Freeing R as a
diagnostic returns R = 0.294, close to the literature value. In contrast,
directly transferring the Steinhögl value R = 0.425 to the evaporated-film
dataset increases the RMSE to 2.821 µΩ·cm and the mean relative error to
38.0 %, with systematic overprediction across the full thickness range.
Thus, the FS–MS framework carries across datasets, but the fitted R value
does not transfer directly between damascene wires and evaporated films.

## 8. Sensitivity analysis

![Figure 5. One-at-a-time sensitivity of the predicted resistivity.](../figures/sensitivity.png)
...
![Figure 6. Exploratory comparison of Cu with liner and barrierless Ru.](../figures/ru_case_study.png)
...
FS–MS framework describes both datasets, but the fitted grain-boundary
reflection coefficient is system-specific. Yarimbiyik's literature value
R = 0.32 describes the evaporated-film data with a mean relative error of
7.4 %, whereas directly transferring the Steinhögl value R = 0.425 produces
a systematic overprediction and a mean relative error of 38.0 %.
